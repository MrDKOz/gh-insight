...
import { MS, COLORS } from "../utils/utils";
...
const StatsBar: FunctionComponent<Props> = ({ items }) => {
  const { closedIssues, openIssues, mergedPRs, closedPRs, avgCycle, fastestCycle, slowestCycle } = useMemo(() => {
    ...
    >
      ...
      {openIssues.length > 0 && (
        <Stat
          ...
          darkColor="#f59e0b"
          label="Issues open"
          title="Number of issues still open"
        />
      )}
      <Stat
        value={String(mergedPRs.length)}
        lightColor={COLORS.prMerged}
        label="PRs merged"
        title="Number of pull requests that have been merged"
      />
      {closedPRs.length > 0 && (
        <Stat
          value={String(closedPRs.length)}
          lightColor={COLORS.prClosed}
          label="PRs closed"
          title="Number of pull requests closed without being merged"
        />
      )}
      {avgCycle !== null && (
        <>
          ...
          <Stat
            value={`${avgCycle}d`}
            label="Avg cycle"
            title="Average days from issue creation to close, across all closed issues"
          />
          <Stat
